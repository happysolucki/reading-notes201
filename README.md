## About Me

Hello, my name is Brandon Keefer. I've never had a real job but I have helped my uncle with some electrician work for
his business. Definitely looking forward to accruing some new skills through this course.

### Table of Contents

#### Code 201 Reading Notes

1. [Tidbits pertaining to HTML, CSS, & Javascript](notes/class-02.md)
2. [HTML Lists, CSS Box Model, & JS Control Flow](notes/listboxcontrol.md)
3. [HTML Links, CSS Layout, & JS Functions](notes/linklayoutfunc.md)
4. [HTML Images & CSS Color/Text](notes/imgcoltxt.md)
5. [JS Object Literals & the DOM](notes/objlitdom.md)
6. [HTML Tables & JS Constructor Functions](notes/tableconstructor.md)
7. [CSS Layout](notes/layout.md)
8. [Forms](notes/forms.md)
9. [Error Handling and Debugging](notes/errorhandling.md)
10. [Audio, Video, and Images](./notes/audiovidimg.md)
11. [Chart.js](./notes/chartjs.md)
12. [Local Storage](./notes/lstorage.md)
13. [CSS Transforms, Transitions, and Animations](./notes/transforms.md)

#### [Github](https://github.com/happysolucki)
